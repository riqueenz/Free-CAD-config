...
class lesson(object):
    def __init__(self):
        ...
        self.data_tutorial = {}
        self.data_tutorial["title"] = app.Qt.translate("MOOC", 'Criando a primeira peça')
        self.data_tutorial["description"] = app.Qt.translate("MOOC", '''Nessa lição você aprenderá a modelisar sua primeira peça no FreeCAD Mechanical.''')
        url = 'https://youtu.be/u8otDF_C_fw'
        ...
        step["description"] = app.Qt.translate("MOOC", '''<h3>Preparação</h3>
            <p><img src= %s width="25"/> Criar um novo documento :
            <ul><li>Abra o menu <i>Arquivo</i> e depois clique em <i>Novo.</li>
            <li>Com o atalho Ctrl + N</li></ul></p>
            <p><img src= %s width="25"/> Abra a aba <b>Modelo 3D</b> :
            <p><img src= %s width="25"/>Escolha o seu estilo de navegação:
            <ul><li>Clique com o botão direito na área de trabalho e abra o menu estilos de navegação</li>
            <li>Blender -> Similar ao Dassault SolidWorks ou Autodesk Inventor</li> \
            <li>CAD -> Similiar ao Dassault Catia</li> \
            <li>Touchpad -> Facilita a utilização em notebooks</li> \
            </ul></p>
            <p><b>Aqui está a peça que vamos modelisar:</b> <br>
            <img src= %s width=340 /></p>''') % (img1, img2, img3, img4)

        step["video"] = str(url) + str('?t=1171')
        step["objectives"] = [
            app.Qt.translate("MOOC", "Criar um novo documento."),
            app.Qt.translate("MOOC", "Abrir a aba modelo 3D."),
            app.Qt.translate("MOOC", "Estilo de navegação: Blender")]
        step["checks"] = [
            "MoocChecker.document_presence()",
            "MoocChecker.active_workbench('PartDesignWorkbench')",
            "MoocChecker.navigation_style('Gui::BlenderNavigationStyle')", ]
        self.data_tutorial["steps"].append(step)

        # Step 2
        step = {}
        img1 = os.path.join(moocWB_icons_path, 'PartDesign_Body.png')
        step["description"] = app.Qt.translate("MOOC", '''<h3>Corpo de peça</h3>
            <p>Para modelizar um sólido precisamos de um corpo que contenha todas as nossas operações.</p>
            <p><img src= %s width="25"/>Para criar um novo corpo de peça:
